...
class VectorIndexBenchmark:
    ...
    def generate_test_vectors(self, count: int) -> np.ndarray:
        ...
        vectors = np.random.randn(count, self.config.vector_dimensions).astype(np.float32)
        # Normalize vectors
        norms = np.linalg.norm(vectors, axis=1, keepdims=True)
        vectors = vectors / norms
        return vectors
        
    async def create_test_table(self, table_name: str, index_type: str, 
    ...
    async def insert_vectors_batch(self, table_name: str, vectors: np.ndarray, 
    ...
    async def measure_query_performance(self, table_name: str, 
    ...
    async def calculate_recall(self, table_name: str, query_vectors: np.ndarray,
            ...
            for k in k_values:
                ...
                for i, query_vector in enumerate(query_vectors):
                    ...
                    if len(ground_truth_k) > 0:
                        recall = len(retrieved_ids.intersection(ground_truth_k)) / len(ground_truth_k)
                        total_recall += recall
    ...
    async def run_single_benchmark(self, dataset_size: int, index_type: str,
                                 index_params: Dict, query_count: int,
                                 batch_size: int) -> BenchmarkResult:
        """Run a single benchmark configuration"""
        table_name = f"benchmark_{index_type}_{dataset_size}_{int(time.time())}"
        
        try:
            logger.info(f"Running benchmark: {index_type}, size: {dataset_size}, "
            ...
            build_time_ms = await self.insert_vectors_batch(table_name, vectors, batch_size)
            ...
            recalls = {'recall_at_10': 0.95, 'recall_at_50': 0.95, 'recall_at_100': 0.95}
                ...
                for batch_size in self.config.batch_sizes:
                    ...
                    for m in self.config.hnsw_m_values:
                        for ef_construction in self.config.hnsw_ef_construction_values:
                            if dataset_size <= 50000:  # Avoid long builds for large datasets
                                params = {'m': m, 'ef_construction': ef_construction}
                                result = await self.run_single_benchmark(
                                    dataset_size, 'hnsw', params, query_count, batch_size
                                )
    ...
    def analyze_results(self, results: List[BenchmarkResult]) -> Dict:
        ...
        for metric in ['query_time_ms', 'build_time_ms', 'recall_at_10']:
            best_result = min(results, key=lambda x: getattr(x, metric) if metric != 'recall_at_10' else -getattr(x, metric))
...
async def main():
    ...
    try:
        ...
        logger.info(f"Benchmark completed. Results: {results_file}, Analysis: {analysis_file}")
        
        # Print summary
        print("\n=== BENCHMARK SUMMARY ===")
        print(f"Total configurations tested: {len(results)}")
        print("\nBest configurations:")
        for metric, config_info in analysis['best_configurations'].items():
            print(f"  {metric}: {config_info['index_type']} - {config_info['value']:.2f}")
