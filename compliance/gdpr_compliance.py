...
class GDPRComplianceManager:
        ...
        self.default_processing_purposes = [
            {
                ...
                'data_categories': [DataCategory.IDENTITY, DataCategory.CONTACT, DataCategory.TECHNICAL],
    ...
    async def _create_compliance_tables(self):
        ...
        await self.db.client.rpc('create_table_if_not_exists', {
    ...
    async def record_consent(
        ...
        if processing_records:
            update_data = {
                'consent_given': consent_given,
                'consent_withdrawn_at': datetime.utcnow().isoformat() if not consent_given else None
            }
            await self.db.secure_update(
                'data_processing_records',
                processing_records[0]['id'],
                update_data
            )
    ...
    async def get_user_consents(self, user_id: uuid.UUID) -> List[Dict[str, Any]]:
        ...
        return await self.db.secure_select(
            'consent_history',
            {'user_id': str(user_id)}
        )
    ...
    async def process_access_request(self, user_id: uuid.UUID) -> Dict[str, Any]:
        ...
        tables_to_export = [
            'users', 'memories', 'api_keys', 'sessions', 
            'audit_logs', 'data_processing_records', 'consent_history'
        ]
        ...
        user_data['data_processing_info'] = {
            'purposes': [record['purpose'] for record in user_data.get('data_processing_records', [])],
    ...
    async def process_deletion_request(self, user_id: uuid.UUID, verified: bool = False):
        ...
        deletion_results = {}
        tables_to_delete = [
            'consent_history', 'data_subject_requests', 'data_processing_records',
            'audit_logs', 'sessions', 'api_keys', 'memories'
        ]
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        expired_records = await self.db.client.table('data_retention_schedule').select('*').lte(
            'retention_until', datetime.utcnow().isoformat()
        ).eq('deletion_scheduled', False).execute()
    ...
    async def generate_privacy_impact_assessment(self) -> Dict[str, Any]:
        ...
        pia_report = {
            ...
            'data_subject_rights': {
                ...
                'average_response_time_hours': request_stats.get('avg_response_time', 0)
            },
            'data_retention': {
        ...
        try:
            if request_type == 'access':
                ...
                update_data = {
                    'status': 'completed',
                    'completed_at': datetime.utcnow().isoformat(),
                    'data_provided': {'download_available': True, 'size_bytes': len(data_bytes)}
                }
            else:
                return  # Manual processing required
            
            await self.db.secure_update('data_subject_requests', str(request_id), update_data)
    ...
    async def _generate_privacy_recommendations(self) -> List[str]:
        ...
        if consent_stats.get('consent_rate', 1.0) < 0.8:
            recommendations.append("Improve consent mechanisms - current consent rate is below 80%")
        
        # Check data retention
        expired_data = await self.db.client.table('data_retention_schedule').select('count').lte(
            'retention_until', datetime.utcnow().isoformat()
        ).eq('deletion_scheduled', False).execute()
        
        if expired_data.count > 100:
            recommendations.append("Schedule cleanup of expired data - large backlog detected")
