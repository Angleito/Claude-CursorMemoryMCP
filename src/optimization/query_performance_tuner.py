...
class PostgreSQLOptimizer:
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                if self.config.enable_auto_explain:
                    ...
                    await conn.execute(
                        f"SET auto_explain.log_min_duration = {self.config.auto_explain_min_duration}"
                    )
    ...
    async def analyze_index_performance(self) -> list[IndexAnalysis]:
        ...
        async with self.pool.acquire() as conn:
            ...
            # Combine data
            size_map = {
                (row["schemaname"], row["indexname"]): row["size_bytes"]
                for row in index_sizes
            }
            def_map = {
                (row["schemaname"], row["indexname"]): row["indexdef"]
    ...
    def _calculate_index_efficiency(self, stats: dict, size_bytes: int) -> float:
        ...
        fetch_ratio = tuples_fetched / max(
            tuples_read, 1
        )  # How many tuples actually used
        size_factor = max(
            0, 1 - (size_bytes / (100 * 1024 * 1024))
    ...
    def _generate_index_recommendations(
        ...
        if index_type == "HNSW":
            ...
            recommendations.append(
                "Consider adjusting lists parameter based on data size"
            )
        ...
        async with self.pool.acquire() as conn:
            ...
            for setting in current_settings:
                ...
                if name == "shared_buffers":
                    ...
                    recommended_mb = max(
                        64, int(memory_info.total / (1024 * 1024) * 0.05)
                    )
                    current_mb = self._parse_memory_setting(current_value, unit)
    ...
    async def generate_performance_report(self, user_id: str | None = None) -> dict[str, Any]:
        ...
        report = {
            ...
            "index_analysis": [
                asdict(idx) for idx in await self.analyze_index_performance()
            ],
            "recommendations": [],
        }
        ...
        if user_id:
            ...
            similarity_analysis = await self.optimize_similarity_search(
                user_id, test_embedding
            )
        ...
        unused_indexes = [
            idx for idx in report["index_analysis"] if idx["usage_count"] == 0
        ]
        if unused_indexes:
...
async def main():
    ...
    try:
        ...
        for _idx in report["index_analysis"][:5]:  # Show top 5
            pass

        for _i, _rec in enumerate(report["overall_recommendations"][:5], 1):
            pass

        if "similarity_search_analysis" in report:
            search_analysis = report["similarity_search_analysis"]
            for _query_name, metrics in search_analysis.items():
