        ...
        try:
            while self.is_alive and self.state == ConnectionState.CONNECTED:
                ...
                try:
                    ...
                    logger.error("Error in ping loop", error=str(e), user_id=self.user_id)
                    await manager.disconnect(self.websocket)
                    break
            ...
            logger.error("Unexpected error in ping loop", error=str(e), user_id=self.user_id)
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: Optional[str] = None, connection_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.error("Failed to accept WebSocket connection", error=str(e), user_id=user_id)
            try:
    ...
    async def disconnect(self, websocket: WebSocket, code: int = 1000, reason: str = "Normal closure"):
        ...
        to_remove = [conn_id for conn_id, ws in self.connection_by_id.items() if ws == websocket]
    ...
    async def send_personal_message(self, message: Dict[str, Any], websocket: WebSocket) -> bool:
        ...
        try:
            ...
            logger.error("Failed to send personal message", error=str(e), user_id=conn_info.user_id)
            
            # Increment error count
            if conn_info.increment_error():
                logger.warning("Max errors reached, disconnecting", user_id=conn_info.user_id)
    ...
    async def cleanup_stale_connections(self):
        ...
        for websocket, conn_info in list(self.active_connections.items()):
            ...
            if conn_info.last_ping and conn_info.last_pong:
                if conn_info.last_ping > conn_info.last_pong and \
                   (current_time - conn_info.last_ping) > conn_info.ping_timeout:
    ...
    async def handle_message(self, websocket: WebSocket, message: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error("Invalid JSON message", error=str(e), user_id=conn_info.user_id)
                return False
            ...
            try:
                ...
                logger.error("Invalid message structure", error=str(e), user_id=conn_info.user_id)
                return False
            ...
            if ws_message.type == "pong":
                ...
                logger.debug("Received message", type=ws_message.type, user_id=conn_info.user_id)
                return True
                
        except Exception as e:
            logger.error("Error handling WebSocket message", error=str(e), user_id=conn_info.user_id)
            if conn_info.increment_error():
                await self.disconnect(websocket, code=1011, reason="Too many errors")
            return False
...
async def connection_cleanup_task(connection_manager: ConnectionManager):
    ...
    logger.warning("connection_cleanup_task is deprecated - use ConnectionManager._background_cleanup instead")
