...
from .models import (
    MemoryCreate, MemoryResponse, MemorySearch, MemoryUpdate,
    UserCreate, UserResponse, MemoryType, MemoryPriority
)
...
class EmbeddingGenerator:
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        if len(text) > 8000:  # OpenAI's typical limit
            text = text[:8000]
        ...
        try:
            ...
            if not self._validate_embedding(embedding):
                raise ValueError(f"Invalid embedding: expected {self.settings.vector_dimension} dimensions")
            ...
            if "rate_limit_exceeded" in str(e).lower():
                ...
                logger.error("Invalid OpenAI request", error=str(e), text_length=len(text))
...
class MemoryManager:
    ...
    async def create_memory(self, memory_data: MemoryCreate, user_id: str) -> MemoryResponse:
        ...
        try:
            ...
            if len(memory_data.content) > self.settings.max_memory_size:
                raise ValueError(f"Memory content exceeds maximum size of {self.settings.max_memory_size}")
    ...
    async def get_memory(self, memory_id: str, user_id: str) -> Optional[MemoryResponse]:
    ...
    async def search_memories(self, search_data: MemorySearch, user_id: str) -> List[MemoryResponse]:
        ...
        try:
            ...
            if search_data.limit <= 0 or search_data.limit > 1000:
                raise ValueError("Limit must be between 1 and 1000")
            
            # Check cache first
            cache_key = None
            if self.redis_client:
                query_hash = hashlib.md5(f"{user_id}:{search_data.query}:{search_data.limit}".encode()).hexdigest()
                ...
                try:
                    ...
                    if cached_result:
                        ...
                        return [MemoryResponse(**item) for item in cached_data]
                except Exception as e:
                    logger.warning("Cache retrieval failed", error=str(e))
            
            # Generate query embedding
            query_embedding = await self.embedding_generator.generate_embedding(search_data.query.strip())
            ...
            params.append(min(search_data.limit, 1000))  # Enforce maximum limit
            ...
            for row in results:
                try:
                    ...
                    logger.warning("Failed to parse memory result", error=str(e), memory_id=row.get('id'))
                    continue
            ...
            await self._log_search(user_id, search_data.query, len(memories), execution_time)
    ...
    async def update_memory(self, memory_id: str, update_data: MemoryUpdate, user_id: str) -> Optional[MemoryResponse]:
            ...
            if success:
                if self.redis_client:
                    try:
                        ...
                        logger.warning("Failed to remove memory from cache", error=str(e))
                
                self._stats['memories_deleted'] += 1
                logger.info("Memory deleted", memory_id=memory_id, user_id=user_id)
            else:
                logger.warning("Memory not found or not owned by user", 
    ...
    async def _log_search(self, user_id: str, query: str, results_count: int, execution_time_ms: float):
