...
class MemoryCreate(BaseModel):
    ...
    content: str = Field(..., description="Memory content", min_length=1, max_length=100000)
    metadata: Optional[Dict[str, Any]] = Field(default_factory=dict)
    tags: Optional[List[str]] = Field(default_factory=list)
    memory_type: MemoryType = Field(MemoryType.FACT)
    priority: MemoryPriority = Field(MemoryPriority.MEDIUM)
    source: Optional[str] = Field(None, description="Source of the memory", max_length=1000)
    context: Optional[str] = Field(None, description="Additional context", max_length=10000)
...
class MemoryUpdate(BaseModel):
    ...
    def validate_update_fields(cls, values):
        ...
        update_fields = ['content', 'metadata', 'tags', 'memory_type', 'priority', 'context']
