...
class Settings(BaseModel):
    ...
    host: str = Field("127.0.0.1", env="HOST")  # Default to localhost for security
    ...
    def validate_secret_key(cls, v: str) -> str:
        ...
        if len(v) < MIN_OPENAI_KEY_LENGTH:
    ...
    def validate_token_expire_minutes(cls, v: int) -> int:
        ...
        if v <= 0 or v > MAX_SESSION_LIFETIME_SECONDS:  # Max 30 days
    ...
    def validate_max_memory_size(cls, v: int) -> int:
        ...
        if v <= 0 or v > MAX_FILE_SIZE_BYTES:  # 10MB limit
    ...
    def validate_memory_ttl_days(cls, v: int) -> int:
        """Validate memory TTL."""
        if v <= 0 or v > MAX_RETENTION_DAYS:  # Max 10 years
