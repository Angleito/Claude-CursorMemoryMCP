...
class MCPServer:
    ...
    async def initialize(self, client_info: Optional[Dict[str, Any]] = None):
    ...
    async def handle_request(self, request: Dict[str, Any], user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Handling MCP request", method=method, user_id=user_id, request_id=request_id)
            
            # Check if server is initialized for methods other than initialize
            if method != "initialize" and not self._initialized:
                return self._create_error_response(
                    request_id, MCPErrorCode.INITIALIZATION_FAILED, 
            ...
            # Legacy memory operations (for backwards compatibility)
                ...
                return self._create_error_response(
                    request_id, MCPErrorCode.METHOD_NOT_FOUND, 
            ...
            logger.error("Validation error in MCP request", error=str(e), user_id=user_id, request_id=request_id)
            return self._create_error_response(
                request_id, MCPErrorCode.INVALID_PARAMS, str(e)
            )
        except Exception as e:
            logger.error("Error handling MCP request", error=str(e), user_id=user_id, request_id=request_id)
            return self._create_error_response(
                request_id, MCPErrorCode.INTERNAL_ERROR, 
                "Internal server error", {"type": type(e).__name__}
            )
    
    def _create_error_response(self, request_id: Optional[str], error_code: MCPErrorCode, 
    ...
    async def _handle_initialize(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        protocol_version = params.get("protocolVersion", MCPProtocolVersion.DEFAULT_VERSION)
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "tags": {"type": "array", "items": {"type": "string"}},
                        "memory_type": {"type": "string", "enum": ["fact", "conversation", "task", "preference", "skill", "context"]},
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "tags": {"type": "array", "items": {"type": "string"}},
                        "sort_by": {"type": "string", "enum": ["created_at", "updated_at", "access_count"], "default": "created_at"},
    ...
    async def _handle_read_resource(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        if uri == "memory://search":
            ...
            try:
                ...
                raise ValueError(f"Failed to get memory stats: {str(e)}")
        else:
    ...
    async def _handle_tool_call(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _tool_memory_search(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _tool_memory_create(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        """Execute memory create tool"""
        try:
            memory_data = MemoryCreate(**args)
        except Exception as e:
            raise ValueError(f"Invalid memory creation parameters: {str(e)}")
        ...
        return {
            "content": [
                {
                    "type": "text", 
                    "text": f"Created memory: {memory.content[:100]}..."
                }
            ],
            "isError": False,
    ...
    async def _tool_memory_get(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _tool_memory_list(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Failed to list memories: {str(e)}")
    ...
    async def _tool_memory_update(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        memory = await self.memory_manager.update_memory(memory_id, update_data, user_id)
        if not memory:
            return {
                "content": [{"type": "text", "text": f"Memory {memory_id} not found"}],
                "isError": True
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Updated memory: {memory.content[:100]}..."
                }
            ],
            "isError": False,
            "data": {"memory": memory.dict()}
    ...
    async def _tool_memory_delete(self, args: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_memory_create(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_memory_search(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_memory_get(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_memory_list(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        """Direct memory list handler"""
        return await self._tool_memory_list(params, user_id)
    
    async def _handle_memory_update(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_memory_delete(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def _handle_subscribe_notifications(self, params: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    async def sse_stream(self, user_id: str) -> AsyncGenerator[str, None]:
        ...
        # Create cleanup task
        cleanup_task = asyncio.create_task(self._cleanup_stream_after_timeout(stream_id, 3600))  # 1 hour timeout
        ...
        try:
            ...
            while not self._closed:
                try:
                    ...
                    logger.warning("Stream queue full, dropping oldest events", stream_id=stream_id)
    ...
    async def broadcast_event(self, event: Dict[str, Any], user_id: Optional[str] = None):
        ...
        for stream_id, queue in self.active_streams.items():
            if user_id is None or user_id in stream_id:
                try:
                    ...
                    logger.warning("Stream queue full, dropping event", stream_id=stream_id)
                    failed_streams.append(stream_id)
                except Exception as e:
                    logger.error("Failed to send event to stream", error=str(e), stream_id=stream_id)
                    failed_streams.append(stream_id)
    ...
    async def close(self):
        ...
        for stream_id, queue in list(self.active_streams.items()):
            try:
                ...
                logger.warning("Failed to send close event to stream", stream_id=stream_id, error=str(e))
            finally:
                del self.active_streams[stream_id]
