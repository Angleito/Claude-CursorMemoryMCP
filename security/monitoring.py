...
class SecurityMetricsCollector:
    ...
    def get_metric_trend(self, metric_name: str, window_minutes: int = 60) -> dict[str, Any]:
        ...
        if len(recent_metrics) < MIN_METRICS_FOR_TREND:
    ...
    def _load_default_alert_rules(self) -> None:
        ...
        default_rules = [
            AlertRule(
                ...
                name="High CPU Usage",
                condition=lambda data: data.get("cpu_usage", 0) > CPU_USAGE_THRESHOLD,
            ...
            AlertRule(
                ...
                condition=lambda data: data.get("failed_login_attempts", 0) > FAILED_LOGIN_THRESHOLD,
            ...
            AlertRule(
                ...
                condition=lambda data: data.get("requests_per_minute", 0) > REQUESTS_PER_MINUTE_THRESHOLD,
            ...
            AlertRule(
                ...
                condition=lambda data: data.get("malicious_ip_requests", 0) > MALICIOUS_IP_THRESHOLD,
                severity=AlertSeverity.HIGH,
            ...
            AlertRule(
                ...
                name="Low Disk Space",
                condition=lambda data: data.get("disk_usage", 0) > DISK_USAGE_THRESHOLD,
                severity=AlertSeverity.MEDIUM,
            ...
            AlertRule(
                ...
                condition=lambda data: data.get("memory_usage", 0) > MEMORY_USAGE_THRESHOLD,
                severity=AlertSeverity.HIGH,
    ...
    async def _send_notifications(self, alert: SecurityAlert) -> None:
        ...
        for channel in self.notification_channels:
            try:
                ...
                logger.error("Failed to send %s notification: %s", channel['type'], e)
...
Please investigate this alert promptly.
    ...
    def get_active_alerts(self, severity_filter: AlertSeverity | None = None) -> list[SecurityAlert]:
    ...
    def get_alert_statistics(self, days: int = 7) -> dict[str, Any]:
        ...
        return {
            ...
            "resolution_rate": (resolved_count / total_alerts * 100) if total_alerts > MIN_TOTAL_ALERTS else MIN_TOTAL_ALERTS,
...
class IncidentManager:
    ...
    async def create_incident(self, title: str, description: str,
                            severity: AlertSeverity, alerts: list[str] | None = None) -> str:
...
class SecurityMonitoringManager:
    ...
    async def _auto_create_incident(self, alert: SecurityAlert) -> None:
        ...
        logger.info("Auto-created incident %s for critical alert %s", incident_id, alert.alert_id)
