...
from security.encryption import encryption_manager, database_encryption
...
class SecureSupabaseClient:
    ...
    async def secure_insert(
        ...
        if result.data:
            ...
            raise Exception(f"Failed to insert into {table}: {result}")
    ...
    async def secure_select(
        ...
        if result.data:
            # Decrypt results
            decrypted_results = [
                self._decrypt_data(table, row) for row in result.data
            ]
            
            # Log operation
            await self._log_operation(
                'select', table, None, user_id, {'filters': filters, 'count': len(decrypted_results)}
            )
    ...
    async def secure_update(
        ...
        # Update via Supabase
        result = self.client.table(table).update(encrypted_data).eq('id', record_id).execute()
        ...
        if result.data:
            ...
            raise Exception(f"Failed to update {table} record {record_id}")
    ...
    async def secure_delete(
        ...
        # Log operation
        await self._log_operation(
            'delete', table, record_id, user_id, {'deleted_record': existing[0] if existing else None}
        )
    ...
    async def secure_vector_search(
        ...
        encrypted_vector = self.encryption.encrypt_vector(vector)
        
        # Use parameterized SQL for vector operations (no table name interpolation)
        if table not in ['memories', 'mem0_vectors.memories']:  # whitelist allowed tables
        ...
        async with self.pool.acquire() as conn:
            ...
            for row in rows:
                ...
                if 'metadata' in result and result['metadata']:
                    result['metadata'] = self.encryption.decrypt_json(result['metadata'])
                
                results.append(result)
        
        # Log operation
        await self._log_operation(
            'vector_search', table, None, user_id, 
            {'vector_length': len(vector), 'limit': limit, 'results_count': len(results)}
        )
    ...
    async def create_encrypted_backup(
        ...
        # Log backup creation
        await self._log_operation(
            'backup', 'system', None, user_id, 
            {'tables': tables, 'record_count': sum(len(data) for data in backup_data.values())}
        )
    ...
    async def restore_encrypted_backup(
        ...
        try:
            ...
            # Log restore
            await self._log_operation(
                'restore', 'system', None, user_id, 
                {'tables': list(backup_data.keys()), 'total_records': sum(len(records) for records in backup_data.values())}
            )
    ...
    def _encrypt_filters(self, table: str, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in filters.items():
            ...
            if key in encrypted_fields:
                ...
                try:
                    encrypted_filters[f"{key}_hash"] = self.db_encryption.get_encrypted_search_hash(str(value), key)
                except Exception as e:
    ...
    async def _log_operation(
        ...
        for key, value in details.items():
            if isinstance(key, str) and len(key) <= 50 and key.replace('_', '').isalnum():
                # Truncate large values and sanitize
                if isinstance(value, str):
                    sanitized_details[key] = value[:500] if len(value) <= 500 else value[:500] + "...[truncated]"
    ...
    async def migrate_table_to_encrypted(self, table: str, batch_size: int = 100):
        ...
        async with self.client.pool.acquire() as conn:
            ...
            for offset in range(0, total_records, batch_size):
                ...
                print(f"Migrated {min(offset + batch_size, total_records)}/{total_records} records in {table}")


# Create global secure client instance
async def initialize_secure_database():
