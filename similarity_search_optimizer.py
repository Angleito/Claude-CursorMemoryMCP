...
class SearchCache:
    ...
    def _get_cache_key(self, query: SearchQuery) -> str:
        ...
        key_data = {
            ...
            'metadata_filters': json.dumps(query.metadata_filters or {}, sort_keys=True),
    ...
    async def set(self, query: SearchQuery, response: SearchResponse):
        ...
        try:
            ...
            await self.redis_client.setex(key, query.config.cache_ttl_seconds, cached_data)
...
class FAISSIndexManager:
    ...
    def create_index(self, user_id: str, algorithm: SearchAlgorithm, 
        ...
        with self.lock:
            ...
            logger.info(f"Created FAISS {algorithm.value} index for user {user_id} "
                       f"with {len(embeddings)} vectors")
    
    def search(self, user_id: str, algorithm: SearchAlgorithm, 
              query_embedding: np.ndarray, k: int, nprobe: int = 10) -> Tuple[np.ndarray, np.ndarray]:
        """Search FAISS index"""
        key = f"{user_id}_{algorithm.value}"
        
        if key not in self.indexes:
            raise ValueError(f"No index found for user {user_id} with algorithm {algorithm.value}")
    ...
    def update_index(self, user_id: str, algorithm: SearchAlgorithm, 
...
class SimilaritySearchOptimizer:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            for row in rows:
                result = SearchResult(
                    ...
                    final_score=float(row['similarity_score'])  # Will be updated in reranking
                )
    ...
    def _rerank_results(self, results: List[SearchResult], config: SearchConfig) -> List[SearchResult]:
        ...
        for result in results:
            ...
            last_access_hours = (current_time - result.last_accessed).total_seconds() / 3600
            recency_factor = np.exp(-last_access_hours / (config.temporal_decay_hours / 4))
    ...
    def _apply_diversity(self, results: List[SearchResult], diversity_lambda: float) -> List[SearchResult]:
        ...
        while remaining and len(selected) < len(results):
            ...
            for i, candidate in enumerate(remaining):
                ...
                for selected_result in selected:
                    ...
                    sim = len(set(candidate.memory_text.lower().split()) & 
                             set(selected_result.memory_text.lower().split())) / \
                          len(set(candidate.memory_text.lower().split()) | 
                              set(selected_result.memory_text.lower().split()))
                    max_sim_to_selected = max(max_sim_to_selected, sim)
                
                # MMR score
                mmr_score = (1 - diversity_lambda) * candidate.final_score - \
                           diversity_lambda * max_sim_to_selected
    ...
    async def search(self, query: SearchQuery) -> SearchResponse:
        ...
        query_id = hashlib.md5(
            f"{query.user_id}_{time.time()}_{query.query_embedding[:5]}".encode()
        ).hexdigest()
        ...
        try:
            if query.config.algorithm == SearchAlgorithm.EXACT:
            ...
            elif query.config.algorithm in [SearchAlgorithm.FAISS_FLAT, 
                ...
                raise ValueError(f"Unsupported search algorithm: {query.config.algorithm}")
...
async def main():
    ...
    try:
        ...
        query_embedding = np.random.rand(1536).tolist()  # Random embedding for demo
        ...
        print(f"Found {response.total_found} results in {response.search_time_ms:.2f}ms")
        print(f"Algorithm used: {response.algorithm_used.value}")
        print(f"Cache hit: {response.cache_hit}")
        
        for i, result in enumerate(response.results[:5]):
            print(f"\nResult {i+1}:")
            print(f"  Text: {result.memory_text[:100]}...")
