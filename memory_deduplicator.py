...
class MemoryDeduplicator:
    ...
    async def find_duplicates(self, user_id: str) -> List[DuplicateGroup]:
        ...
        logger.info(f"Found {len(duplicate_groups)} duplicate groups in {processing_time:.2f}ms")
        
        return duplicate_groups
    
    async def _find_exact_duplicates(self, memories: List[Dict]) -> List[DuplicateGroup]:
        ...
        for normalized_text, memory_group in text_to_memories.items():
            if len(memory_group) > 1:
                ...
                memory_group.sort(
                    ...
                    reverse=self.config.preserve_latest
                )
    ...
    async def _find_fuzzy_duplicates(self, memories: List[Dict]) -> List[DuplicateGroup]:
        ...
        for i in range(len(memories)):
            ...
            for j in range(i + 1, len(memories)):
                ...
                weights = {
                    SimilarityMetric.LEVENSHTEIN: 0.25,
                    SimilarityMetric.JARO_WINKLER: 0.25,
                    SimilarityMetric.COSINE_TFIDF: 0.35,
                    SimilarityMetric.JACCARD: 0.15
                }
    ...
    async def _find_semantic_duplicates(self, memories: List[Dict]) -> List[DuplicateGroup]:
        ...
        for i in range(len(memories_with_embeddings)):
            ...
            if similar_memories:
                ...
                all_memories.sort(
                    key=lambda m: (m['importance_score'], m['access_count'], m['created_at']),
                    reverse=True
                )
    ...
    async def _find_temporal_duplicates(self, memories: List[Dict]) -> List[DuplicateGroup]:
    ...
    async def _find_hybrid_duplicates(self, memories: List[Dict]) -> List[DuplicateGroup]:
    ...
    def _merge_duplicate_groups(self, groups: List[DuplicateGroup]) -> List[DuplicateGroup]:
        ...
        for cluster in group_clusters:
            if len(cluster) == 1:
                # No merging needed
                merged_groups.append(groups[list(cluster)[0]])
            else:
    ...
    async def deduplicate_memories(self, user_id: str, 
                                 dry_run: bool = True) -> DeduplicationResult:
        ...
        if not dry_run and duplicate_groups:
            duplicates_removed = await self._remove_duplicates(user_id, duplicate_groups)
    ...
    async def _remove_duplicates(self, user_id: str, 
                               duplicate_groups: List[DuplicateGroup]) -> int:
...
async def main():
    ...
    try:
        ...
        print(f"Running deduplication for user: {user_id}")
        ...
        print(f"  Total duplicates: {result.duplicates_found}")
        ...
        if result.duplicate_groups:
            ...
            for i, group in enumerate(result.duplicate_groups[:3]):
                print(f"  Group {i+1}:")
                print(f"    Canonical ID: {group.canonical_id}")
        ...
        # Get stats
        stats = await deduplicator.get_deduplication_stats(user_id)
        print(f"\nDeduplication Statistics:")
        print(f"  Current memories: {stats['current_memory_count']}")
        print(f"  Total operations: {stats['deduplication_operations']}")
        print(f"  Total removed: {stats['total_memories_removed']}")
